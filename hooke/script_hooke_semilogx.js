$(document).ready(function() {
  ...
  var POINT_COLOR = 'rgba(255,0,0,0.9)'; // point color, eg `black` or `rgba(10, 100, 44, 0.8)`
  var POINT_RADIUS = 2; // radius of each data point

  var X_AXIS = 'strain (log)';  // x-axis label and label in tooltip
  var Y_AXIS = 'stress'; // y-axis label and label in tooltip

  var SHOW_GRID = true; // `true` to show the grid, `false` to hide

  // Read data file and create a chart
  d3.csv('./hooke.csv').then(function(rows) {
    ...
    var ctx = document.getElementById('stress-strain-semilogx').getContext('2d');
